...
else
{
	...
	if ( $_POST['query'] == "_all" )
		$query = "SELECT u.user_id, u.f_name, u.l_name, u.email, u.phone
				  FROM user u
	...
	if ( $stmt = $mysqli->prepare( $query ) ) 
	{
		...
		if ( $_POST['query'] != '_all' )
		{
 		...
 		$stmt->execute();
		$stmt->store_result();
		$stmt->bind_result( $user_id, $f_name, $l_name, $email, $phone );
		...
		while ( $stmt->fetch() )
		{
			...
			$results[] = array( "userid" => $user_id, "firstname" => $f_name, "lastname" => $l_name, "email" => $email, "phone" => $phone, "equipment" => $equipment, );
		}
	}
