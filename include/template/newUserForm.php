...
$( '.input_error' ).hide();
	...
	$( '#selectusers_input, #newuser_input, #selectusertype_input, #selectlab_input, #newlab_input' ).hide();
  ...
  $('#usertypemenu>li>a').on( 'click', function(){
     if ( $( this ).text() == "Faculty/Staff ")
	{
		...
		$( '#newuser_input>div>div>input' ).val( '' );
		$( '#selectlab_input, #newlab_input, #newlab_error, #selectlab_error' ).hide();
	}

     else
	{
		$( '#userlist' ).empty();
		$('#selectusers_input, #selectusers_error, #newuser_input, #newuser_error').hide();
